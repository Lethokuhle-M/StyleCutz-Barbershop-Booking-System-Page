<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Rating_Page.aspx.cs" Inherits="StyleCutzBarberShopWebApp.Rating_Page" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div>
        <asp:GridView ID="GridView1" CssClass="gridview" runat="server" AutoGenerateColumns="False" DataKeyNames="appointment_id" DataSourceID="SqlDataSource1" OnSelectedIndexChanged="GridView1_SelectedIndexChanged" CellPadding="4" ForeColor="#333333" GridLines="None">
            <AlternatingRowStyle BackColor="White" />
            <Columns>
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:G7Pmb2024ConnectionString %>" DeleteCommand="DELETE FROM [Appointment_tbl] WHERE [appointment_id] = @original_appointment_id AND [customer_id] = @original_customer_id AND [employee_id] = @original_employee_id AND (([appointment_date] = @original_appointment_date) OR ([appointment_date] IS NULL AND @original_appointment_date IS NULL)) AND (([payment_method] = @original_payment_method) OR ([payment_method] IS NULL AND @original_payment_method IS NULL)) AND [payment_amount] = @original_payment_amount AND (([appointment_timeID] = @original_appointment_timeID) OR ([appointment_timeID] IS NULL AND @original_appointment_timeID IS NULL)) AND [appointment_status] = @original_appointment_status AND (([appointment_rating] = @original_appointment_rating) OR ([appointment_rating] IS NULL AND @original_appointment_rating IS NULL)) AND (([rating_comment] = @original_rating_comment) OR ([rating_comment] IS NULL AND @original_rating_comment IS NULL)) AND (([appointment_location] = @original_appointment_location) OR ([appointment_location] IS NULL AND @original_appointment_location IS NULL)) AND (([booking_platform] = @original_booking_platform) OR ([booking_platform] IS NULL AND @original_booking_platform IS NULL)) AND (([rating_status] = @original_rating_status) OR ([rating_status] IS NULL AND @original_rating_status IS NULL))" InsertCommand="INSERT INTO [Appointment_tbl] ([customer_id], [employee_id], [appointment_date], [payment_method], [payment_amount], [appointment_timeID], [appointment_status], [appointment_rating], [rating_comment], [appointment_location], [booking_platform], [rating_status]) VALUES (@customer_id, @employee_id, @appointment_date, @payment_method, @payment_amount, @appointment_timeID, @appointment_status, @appointment_rating, @rating_comment, @appointment_location, @booking_platform, @rating_status)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT Appointment_tbl.appointment_id, Customer_tbl.customer_name, Employee_tbl.employee_name, Appointment_tbl.appointment_date, Appointment_tbl.payment_amount, Times.TimeSlot FROM Appointment_tbl INNER JOIN Employee_tbl ON Appointment_tbl.employee_id = Employee_tbl.employee_id INNER JOIN Customer_tbl ON Appointment_tbl.customer_id = Customer_tbl.customer_id INNER JOIN Times ON Appointment_tbl.appointment_timeID = Times.SlotID WHERE (Appointment_tbl.appointment_status = @appointment_status) AND (Appointment_tbl.customer_id = @customer_id) AND (Appointment_tbl.rating_status IS NULL)" UpdateCommand="UPDATE [Appointment_tbl] SET [customer_id] = @customer_id, [employee_id] = @employee_id, [appointment_date] = @appointment_date, [payment_method] = @payment_method, [payment_amount] = @payment_amount, [appointment_timeID] = @appointment_timeID, [appointment_status] = @appointment_status, [appointment_rating] = @appointment_rating, [rating_comment] = @rating_comment, [appointment_location] = @appointment_location, [booking_platform] = @booking_platform, [rating_status] = @rating_status WHERE [appointment_id] = @original_appointment_id AND [customer_id] = @original_customer_id AND [employee_id] = @original_employee_id AND (([appointment_date] = @original_appointment_date) OR ([appointment_date] IS NULL AND @original_appointment_date IS NULL)) AND (([payment_method] = @original_payment_method) OR ([payment_method] IS NULL AND @original_payment_method IS NULL)) AND [payment_amount] = @original_payment_amount AND (([appointment_timeID] = @original_appointment_timeID) OR ([appointment_timeID] IS NULL AND @original_appointment_timeID IS NULL)) AND [appointment_status] = @original_appointment_status AND (([appointment_rating] = @original_appointment_rating) OR ([appointment_rating] IS NULL AND @original_appointment_rating IS NULL)) AND (([rating_comment] = @original_rating_comment) OR ([rating_comment] IS NULL AND @original_rating_comment IS NULL)) AND (([appointment_location] = @original_appointment_location) OR ([appointment_location] IS NULL AND @original_appointment_location IS NULL)) AND (([booking_platform] = @original_booking_platform) OR ([booking_platform] IS NULL AND @original_booking_platform IS NULL)) AND (([rating_status] = @original_rating_status) OR ([rating_status] IS NULL AND @original_rating_status IS NULL))">
            ...
            <SelectParameters>
                <asp:Parameter DefaultValue="completed" Name="appointment_status" Type="String" />
                <asp:ControlParameter ControlID="DetailsView1" DefaultValue="" Name="customer_id" PropertyName="SelectedValue" Type="Int32" />
            </SelectParameters>
            <UpdateParameters>
                <asp:Parameter Name="customer_id" Type="Int32" />
                <asp:Parameter Name="employee_id" Type="Int32" />
