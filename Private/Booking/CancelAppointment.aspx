<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="CancelAppointment.aspx.cs" Inherits="StyleCutzBarberShopWebApp.Private.Booking.CancelAppointment" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <h1  style="text-align:center">Cancel Appointments</h1>
    ...
    <div class="gridview-container">
        <asp:GridView ID="GridView1" runat="server" CssClass="gridview gridview-spacing" AutoGenerateColumns="False" DataKeyNames="appointment_id" DataSourceID="SqlDataSource6" OnSelectedIndexChanged="GridView1_SelectedIndexChanged" style="margin-right: 0" Width="1135px" ToolTip="Select Appointment">
            <Columns>
    ...
    <asp:Button ID="btn1" runat="server" OnClientClick ="return Confirm();" CssClass ="btn" OnClick="btn1_Click" Text="Cancel" Width="119px" />
    ...
    <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:G7Pmb2024ConnectionString %>" SelectCommand="SELECT Appointment_tbl.appointment_id, Customer_tbl.customer_name, Appointment_tbl.appointment_date, Appointment_tbl.payment_method, Appointment_tbl.payment_amount, Times.TimeSlot, Appointment_tbl.appointment_status FROM Appointment_tbl INNER JOIN Customer_tbl ON Appointment_tbl.customer_id = Customer_tbl.customer_id INNER JOIN Employee_tbl ON Customer_tbl.employee_id = Employee_tbl.employee_id INNER JOIN Times ON Appointment_tbl.appointment_timeID = Times.SlotID WHERE (Appointment_tbl.customer_id = @customer_id) AND (Appointment_tbl.appointment_status = 'pending') AND (Appointment_tbl.appointment_date = @appointment_date)">
        <SelectParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSource6" runat="server" ConnectionString="<%$ ConnectionStrings:G7Pmb2024ConnectionString %>" SelectCommand="SELECT Appointment_tbl.appointment_id, Customer_tbl.customer_name, Appointment_tbl.appointment_date, Appointment_tbl.payment_method, Appointment_tbl.payment_amount, Times.TimeSlot, Appointment_tbl.appointment_status FROM Appointment_tbl INNER JOIN Customer_tbl ON Appointment_tbl.customer_id = Customer_tbl.customer_id INNER JOIN Employee_tbl ON Customer_tbl.employee_id = Employee_tbl.employee_id INNER JOIN Times ON Appointment_tbl.appointment_timeID = Times.SlotID WHERE (Appointment_tbl.customer_id = @customer_id) AND (Appointment_tbl.appointment_status = 'pending')">
        <SelectParameters>
            <asp:Parameter Name="customer_id" Type="Int32" />
        </SelectParameters>
    </asp:SqlDataSource>
