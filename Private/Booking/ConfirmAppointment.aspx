<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="ConfirmAppointment.aspx.cs" Inherits="StyleCutzBarberShopWebApp.Private.Booking.ConfirmAppointment" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <h1 class="select-services-heading">Confirm The Booking</h1>
    <div class="cn">
        <div>
    ...
    <div class="form-container">
        <div class="form-group">
            ...
            <asp:SqlDataSource ID="BookingPlatformDS" runat="server" ConnectionString="<%$ ConnectionStrings:G7Pmb2024ConnectionString %>" SelectCommand="SELECT * FROM [Appointment_tbl]" UpdateCommand="UPDATE Appointment_tbl SET booking_platform = 'online' WHERE (appointment_id = @id)">
                <UpdateParameters>
    ...
    <script>
        document.getElementById('<%= appointmentDate.ClientID %>').addEventListener('change', function () {
    ...
    <!--<asp:Button ID="btn1" runat="server" CssClass="btn" OnClick="btn1_Click" Text="Confirm" Width="119px" />-->
    <div style="display: flex; justify-content: space-between; width: 100%; padding: 10px;">
        ...
        <asp:Button ID="Button2" runat="server" OnClick="btn1_Click" CssClass="btn" OnClientClick="return Confirm();" Text="Confirm" Width="119px" Style="text-align:center" />
        <script>
            function Confirm() {
                if (confirm("Are you sure you want to Make the Appointment?") == true)
                    return true;
                else
                    return false;
            }
    ...
    <asp:SqlDataSource ID="SqlDataSource4" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:G7Pmb2024ConnectionString %>" DeleteCommand="DELETE FROM [Service_Appointment] WHERE [appointment_id] = @original_appointment_id AND [service_id] = @original_service_id" InsertCommand="INSERT INTO [Service_Appointment] ([appointment_id], [service_id]) VALUES (@appointment_id, @service_id)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT [appointment_id], [service_id] FROM [Service_Appointment]">
        <DeleteParameters>
...
SELECT   @AppointmentID=SCOPE_IDENTITY();" OnInserted="SqlDataSource3_Inserted">
        <InsertParameters>
