<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="CancelOrder.aspx.cs" Inherits="StyleCutzBarberShopWebApp.Private.Manager.CancelOrder" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="grid-container">
        <div>
            <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" DataKeyNames="order_id" DataSourceID="orderDS" OnSelectedIndexChanged="GridView1_SelectedIndexChanged" CssClass="table table-striped">
                <Columns>
        ...
        <div>
            ...
            <asp:Button ID="Button1" runat="server" OnClientClick="return Confirm();" OnClick="Button1_Click" Text="Cancel Order" CssClass="button" />
    ...
    <asp:SqlDataSource ID="orderPaymentDS" runat="server" ConnectionString="<%$ ConnectionStrings:G7Pmb2024ConnectionString %>" DeleteCommand="DELETE FROM [Order_tbl] WHERE [order_id] = @original_order_id AND [supplier_id] = @original_supplier_id AND [quantity] = @original_quantity AND [total_price] = @original_total_price AND [order_date] = @original_order_date AND [status] = @original_status" InsertCommand="INSERT INTO [Order_tbl] ([supplier_id], [quantity], [total_price], [order_date], [status]) VALUES (@supplier_id, @quantity, @total_price, @order_date, @status)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT * FROM [Order_tbl]" UpdateCommand="UPDATE Order_tbl SET status = 'cancelled' WHERE (order_id = @id)">
        <DeleteParameters>
