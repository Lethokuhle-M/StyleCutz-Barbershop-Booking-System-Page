<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="SelectSupplier.aspx.cs" Inherits="StyleCutzBarberShopWebApp.Private.Manager.SelectSupplier" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <h1 class="select-barber-heading">Select The Supplier</h1>

    <asp:DetailsView ID="DetailsView2" runat="server" CssClass="details-view" AutoGenerateRows="False" DataKeyNames="employee_id" DataSourceID="SqlDataSource3">
        <Fields>
    ...
    <div class="listview">
        <asp:ListView ID="ListView2" runat="server" DataSourceID="SqlDataSource2" OnItemCommand="ListView2_ItemCommand">
            <ItemTemplate>
                <div class="list-item">
                    <img src='<%# "/Images/" + Eval("supplier_name") + ".jpg?v=" + DateTime.Now.Ticks %>' alt='<%# Eval("supplier_name") %>' />
                    <div class="service-name">
                        <asp:Label ID="lblSupplierName2" runat="server" Text='<%# Eval("supplier_name") %>'></asp:Label>
                    </div>
                    <div class="service-price">
                        <asp:Label ID="lblSupplierCell2" runat="server" Text='<%# Eval("supplier_cellNo") %>'></asp:Label>
                    </div>
                    <div class="service-price">
                        <asp:Label ID="lblSupplierEmail2" runat="server" Text='<%# Eval("supplier_email") %>'></asp:Label>
                    </div>
                    <div class="service-price">
                        <asp:Label ID="lblSupplierGender2" runat="server" Text='<%# Eval("supplier_address") %>'></asp:Label>
                    </div>
                    <asp:LinkButton ID="SelectButton2" runat="server" Text="Select" CommandName="Select" CommandArgument='<%# Eval("supplier_id") %>' CssClass="btn" />
                </div>
            </ItemTemplate>
        </asp:ListView>
    </div>
   ...
   <div style="display: flex; justify-content: space-between; width: 100%; padding: 10px;">
        <asp:Button ID="Button1" runat="server" OnClick="btn1_Click" CssClass="btn" Text="Previous" Width="119px" Style="text-align:center" />
        <asp:Button ID="btn1" runat="server" OnClick="btn2_Click" CssClass="btn" Text="Next" Width="119px" Style="text-align:center" />
    </div>

    <asp:SqlDataSource ID="SqlDataSource2" runat="server" 
        ConnectionString="<%$ ConnectionStrings:G7Pmb2024ConnectionString %>" 
        SelectCommand="SELECT [supplier_id], [supplier_name], [supplier_cellNo], [supplier_email], [supplier_address] FROM [Supplier_tbl] WHERE [supplier_name] != 'N/A'">
    </asp:SqlDataSource>
